...
**Tested on:** Windows 11 Pro, WSL2 Ubuntu 24.04, consumer desktop hardware
...
6. **Windows machines running their own Claude Code need the same cross-machine rules.** If you run a separate Claude Code instance on a Windows desktop, mirror the cross-machine guardrails into `C:\Users\<you>\.claude\CLAUDE.md`. Otherwise the two instances drift on safety rules and the Windows one will not know about the "never push to main" and "always check PR state first" conventions you enforce on your primary Linux host.
